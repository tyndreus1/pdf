...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function generatePDF() {
        ...
        // 1. Prepare invoiceData in the structure expected by PDFGenerator.generatePDF
        const finalInvoiceNumber = document.getElementById('invoice-number')?.textContent || // Get finalized number if available
                                   `PREVIEW-${Date.now().toString().slice(-6)}`; // Or use a preview number

        // Map app's invoice.products and invoice.prices to PDFGenerator's items structure
        const itemsForPDF = invoice.products.map(product => {
            // Find the original product data to get the image path
            // This assumes product.id exists and is valid for lookup in InvoiceDB
            // FIX: Use .get() instead of potentially non-existent .getById()
            const originalProductData = product.isManual
                ? { image: '' } // Manual products likely don't have images in DB
                : InvoiceDB.products.get(parseInt(product.id)); // Parse ID if needed

            const unitPrice = invoice.prices[product.id] || 0;
            const totalPrice = unitPrice * product.quantity;
            return {
                id: product.id, // Keep ID for reference if needed
                name: product.name, // pdfgenerator expects 'name'
                quantity: product.quantity,
                unitPrice: unitPrice,
                totalPrice: totalPrice,
                // ADD IMAGE PATH HERE
                image: (originalProductData ? originalProductData.image : '') || '' // Get image path, handle missing data
            };
        });

        // Create the data object for PDFGenerator
        const invoiceDataForPDF = {
            invoiceNumber: finalInvoiceNumber,
            type: invoice.type,
            currency: invoice.currency,
            customer: invoice.customer, // Assuming customer object has name, address, email, phone
            items: itemsForPDF, // Use the mapped items WITH IMAGE PATH
            subtotal: calculateSubtotal(),
            discount: {
        ...
        // ... (rest of the function remains the same: validation, calling PDFGenerator, saving) ...

        // Validate essential data
        if (!invoiceDataForPDF.customer) {
             alert("Cannot generate PDF: Customer data is missing.");
             console.error("PDF Generation Error: Missing customer data.");
             return;
        }
         if (itemsForPDF.length === 0) {
             alert("Cannot generate PDF: No products have been added to the invoice.");
             console.error("PDF Generation Error: Missing items data.");
             return;
         }


        // 2. Call PDFGenerator.generatePDF
        const result = PDFGenerator.generatePDF(invoiceDataForPDF);

        // 3. Save the document if successful
        if (result && !result.error && result.doc) {
            try {
                result.doc.save(result.fileName);
                console.log(`PDF saved as ${result.fileName}`);
            } catch (saveError) {
                 console.error("Error saving PDF:", saveError);
                 alert("An error occurred while saving the PDF: " + saveError.message);
            }
        } else {
            console.error("PDF generation failed:", result?.message || "Unknown error");
            alert("Failed to generate PDF: " + (result?.message || "Please check console for details."));
        }
    }
